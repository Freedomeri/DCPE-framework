...
outputClean_Dir = args.output

#input_Dir = r'./user_data/processed_data/data_txt_sorted_Aoti_Aoti.txt'
output_Dir = r'./prediction_result/temp.json'
#outputClean_Dir = r'./prediction_result/result_Aoti_Clean_x.json'
...
sorted_test_text_infos = open(input_Dir, 'r', encoding='utf-8').readlines()  # TODO Test B
...
# TODO Ensemble
best_single_model_result = json.load(open("./user_data/result_single_models/result-0.json", 'r', encoding="utf-8"))
...
json.dump(result_json_file_ensemble, open(output_Dir, "w+", encoding="utf-8"), ensure_ascii=False, indent=4)
...
def drop_similar(_data, similarity):
    ...
    for i in range(data_length):
        for j in range(i + 1, i + 25 if (i + 25) < data_length else data_length):
...
data1 = drop_similar(data, similarity=0.66)
...
print("Clean Done")
json.dump(data2, open(outputClean_Dir, "w+", encoding="utf-8"), ensure_ascii=False, indent=4)